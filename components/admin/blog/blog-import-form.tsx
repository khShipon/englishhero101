...
import {
  importBlogPostsCsv,
  importBlogPostsJson,
  type BlogImportState,
} from "@/lib/admin/blog-bulk-import-actions";
import { parseCsv } from "@/lib/admin/csv-import";
import { validateBlogCsvRow } from "@/lib/admin/blog-bulk-import";
...
function CsvImportPanel() {
  const [state, formAction, pending] = useActionState<BlogImportState, FormData>(
    importBlogPostsCsv,
    undefined,
  );
  const [showFormat, setShowFormat] = useState(false);
  const [checks, setChecks] = useState<
    ({ label: string; title: string } & ({ ok: true } | { ok: false; error: string }))[] | null
  >(null);
  ...
  async function handleFileChange(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    setChecks(
      rows.map((row, index) => {
        const result = validateBlogCsvRow(row, index + 2);
        return result.ok
          ? { label: result.label, title: result.data.title, ok: true }
          : { label: result.label, title: row.title || "(missing)", ok: false, error: result.error };
      }),
    );
  }
    ...
    <form action={formAction}>
          ...
          <div className="flex flex-col gap-2">
            <div className="flex flex-wrap items-center gap-2 text-sm">
              ...
              <Badge variant={invalidCount > 0 ? "destructive" : "default"}>
                {checks.length} post{checks.length === 1 ? "" : "s"}
      ...
      <CardFooter>
        <Button type="submit" disabled={pending || !checks || invalidCount > 0}>
          {pending ? "Importing..." : checks ? `Import ${checks.length} post(s)` : "Import"}
        </Button>
      </CardFooter>
    </form>
  );
  ...
  return (
    <form action={formAction}>
      <CardContent className="flex flex-col gap-4">
        ...
        <div className="flex items-center justify-between">
          <Label htmlFor="payload">Post document (JSON)</Label>
        ...
        <Textarea
          ...
          name="payload"
          rows={18}
          placeholder="Paste one post object, or an array of post objects..."
...
export function BlogImportForm() {
  const [mode, setMode] = useState<"csv" | "json">("json");

  return (
    <Card className="max-w-4xl">
      <CardHeader>
        <CardTitle>Import blog posts</CardTitle>
        <CardDescription>
          Bulk-add posts from a JSON or CSV file — generate one with any AI model, starting from
          the template.{" "}
          <Link href="/admin/blog/import/template-json" className="underline">
            Download JSON template
          </Link>{" "}
          ·{" "}
          <Link href="/admin/blog/import/template" className="underline">
